...
ssh $IMAGER_USER@$IMAGER_HOST 'mkdir -p ~/cookbooks'
...
ssh $IMAGER_USER@$IMAGER_HOST 'sudo cp ~/cookbooks/apple_orchard/assets/com.pivotallabs.auto_set_hostname.plist  /Library/LaunchAgents/'
ssh $IMAGER_USER@$IMAGER_HOST 'mkdir ~/bin; sudo cp ~/cookbooks/apple_orchard/assets/auto_set_hostname.rb /usr/sbin/'
